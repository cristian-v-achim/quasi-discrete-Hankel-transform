...
{   
    ...
    cudaMalloc((void**)&d_rfhat, sizeof(double)*(nr));
    cudaMalloc((void**)&d_kfhat, sizeof(double)*(nr));
    cudaMalloc((void**)&d_fhatout, sizeof(double)*(nr));
    for(int i=0;i<nr;i++)
    {
      rfhat[i]=0.0;
    ...
    start = clock(); 
    cudaMemcpy(d_rfhat, rfhat, sizeof(double)*(nr),cudaMemcpyHostToDevice);
    gpu_ht_transf_otpp<<<bpg,tpb>>>(d_rfhat,d_kfhat,mvectors.invmonetwo.device,mvectors.monetwo.device, mvectors.normbessolutions.device,nr);
    
    cudaMemcpy(fhatout, d_kfhat, sizeof(double)*(nr),cudaMemcpyDeviceToHost); 
    ...
    cudaMemset(d_fhatout, 0, sizeof(double)*nr);
    gpu_ht_transf_otpp<<<bpg,tpb>>>(d_kfhat,d_fhatout,mvectors.invmonetwo.device,mvectors.monetwo.device, mvectors.normbessolutions.device,nr);

    cudaMemcpy(fhatout, d_fhatout, sizeof(double)*(nr),cudaMemcpyDeviceToHost); 
...
__host__ void hankelinit(hankelvectors *mvectors,const int nr,const double rmax)
{
   ...
   printf("%lu KB free of total %lu KB at the beginning. \n\n",free/1024,total/1024);
    ...
    cudaMalloc((void**)&mvectors[0].bessolutions.device, sizeof(double)*(nr+1));
    cudaMalloc((void**)&mvectors[0].normbessolutions.device, sizeof(double)*(nr+1));
    cudaMalloc((void**)&mvectors[0].monetwo.device, sizeof(double2)*nr);
    cudaMalloc((void**)&mvectors[0].invmonetwo.device, sizeof(double)*(nr));
    cudaMalloc((void**)&mvectors[0].r.device, sizeof(double)*(nr));
    cudaMalloc((void**)&mvectors[0].k.device, sizeof(double)*(nr));
    ...
    cudaMemcpy(mvectors[0].bessolutions.device, mvectors[0].bessolutions.host, sizeof(double)*(nr+1),cudaMemcpyHostToDevice);
    cudaMemcpy(mvectors[0].normbessolutions.device, mvectors[0].normbessolutions.host, sizeof(double)*(nr+1),cudaMemcpyHostToDevice);
    cudaMemcpy(mvectors[0].monetwo.device, mvectors[0].monetwo.host, sizeof(double2)*(nr),cudaMemcpyHostToDevice);
    cudaMemcpy(mvectors[0].invmonetwo.device, mvectors[0].invmonetwo.host, sizeof(double)*(nr),cudaMemcpyHostToDevice);
    cudaMemcpy( mvectors[0].r.device, mvectors[0].r.host, sizeof(double)*(nr),cudaMemcpyHostToDevice);
    cudaMemcpy( mvectors[0].k.device, mvectors[0].k.host, sizeof(double)*(nr),cudaMemcpyHostToDevice);
